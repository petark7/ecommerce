...
const PaymentOptions = ({ setSelectedOption, formErrors }) => (
  <section
    className={`border rounded mt-3 p-3 ${
      formErrors.paymentOptionError ? "border-2 border-red-300" : "border"
    }`}
  >
    ...
    <div className="mt-2">
      ...
      {formErrors.paymentOptionError && (
        <div className="text-red-500 mt-2">Please select a payment option.</div>
      )}
    </div>
  </section>
);
...
PaymentOptions.propTypes = {
  formErrors: PropTypes.shape({
    paymentOptionError: PropTypes.string,
  }),
